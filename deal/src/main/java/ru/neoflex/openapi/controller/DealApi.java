...
public interface DealApi {
    ...
    default ResponseEntity<Void> _applyOffer(
        @Parameter(name = "LoanOfferDTO", description = "", required = true, schema = @Schema(description = "")) @Valid @RequestBody LoanOfferDTO loanOfferDTO
    ) {
    ...
    @Operation(
        ...
        responses = {
            @ApiResponse(responseCode = "200", description = "Application created", content = @Content(mediaType = "application/json", schema = @Schema(implementation =  LoanOfferDTO.class)))
        }
    )
    @RequestMapping(
    ...
    default ResponseEntity<List<LoanOfferDTO>> _createApplication(
        @Parameter(name = "LoanApplicationRequestDTO", description = "", required = true, schema = @Schema(description = "")) @Valid @RequestBody LoanApplicationRequestDTO loanApplicationRequestDTO
    ) {
        ...
        getRequest().ifPresent(request -> {
            for (MediaType mediaType: MediaType.parseMediaTypes(request.getHeader("Accept"))) {
                if (mediaType.isCompatibleWith(MediaType.valueOf("application/json"))) {
                    String exampleString = "{ \"totalAmount\" : 10150.0, \"isSalaryClient\" : true, \"monthlyPayment\" : 1756.38, \"rate\" : 13, \"requestedAmount\" : 10000.0, \"term\" : 6, \"applicationId\" : 1, \"isInsuranceEnabled\" : true }";
    ...
    default ResponseEntity<Void> _finishRegistration(
        @Parameter(name = "applicationId", description = "Id of application", required = true, schema = @Schema(description = "")) @PathVariable("applicationId") Long applicationId,
        @Parameter(name = "FinishRegistrationRequestDTO", description = "", required = true, schema = @Schema(description = "")) @Valid @RequestBody FinishRegistrationRequestDTO finishRegistrationRequestDTO
    ) {
